...
function pollForVerificationResults() {
  ...
  if (signInfoBlock || reportBtn) {
    ...
    // Filter anchors specifically for the "Save all / Archive / All together" ZIP download
    const zipAnchors = allAnchors.filter(a => {
      const text = (a.innerText || "").toLowerCase();
      const hasZipText = text.includes("зберегти всі") ||
                         text.includes("архів") ||
                         text.includes("все разом") ||
                         text.includes("все в одному") ||
                         text.includes("завантажити архів") ||
                         text.includes("скачати все") ||
                         text.includes("зберегти архівом");

      const hasZipAttr = (a.download && a.download.toLowerCase().endsWith(".zip")) ||
                         (a.href && a.href.toLowerCase().includes(".zip")) ||
                         (a.id && (a.id.toLowerCase().includes("zip") || a.id.toLowerCase().includes("receipt") || a.id.toLowerCase().includes("saveall")));

      return hasZipText || hasZipAttr;
    });

    if (zipAnchors.length > 0) {
      zipAnchor = zipAnchors[0];
    }

    // Check if the ZIP anchor we found has a valid href attribute.
    // If not, we wait up to 5 seconds for it to be generated by the page script.
    const isZipReady = zipAnchor && zipAnchor.href;

    if (!isZipReady && signInfoBlock && signInfoFoundTicks < 5 && resultPollingCount < maxResultPollingCount) {
      signInfoFoundTicks++;
      logToBackground(`Verification block found. Waiting for ZIP archive download button/href to populate (tick ${signInfoFoundTicks}/5)...`);
      setTimeout(pollForVerificationResults, 1000);
      return;
    }

    // Automatically download only the ZIP archive result if found and ready
